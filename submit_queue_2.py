        ...
        for tid, effect in self.effects.items():
            if effect == 0.0:
                ...
                for _ in range(10): 
                    ...
                    if new_effect > 0.0: break
                if new_effect == 0.0: new_effect = 1.0
...
class PipelinedSubmitQueue:
    def __init__(self, test_defs: list[TestDefinition], pipeline_depth: int, max_minibatch_size: int):
        self.test_defs = test_defs
        self.all_test_ids = [t.id for t in test_defs]
        self.repo = RepositoryState()
        for t in test_defs: self.repo.base_p_pass[t.id] = 1.0
    ...
    def add_changes(self, n=1):
        for _ in range(n):
            self.pending_changes.append(Change.create(self.change_id_counter, self.test_defs))
    ...
    def step(self) -> int:
        ...
        for i in range(self.pipeline_depth):
            ...
            minibatches.append(Minibatch(
                changes=list(cumulative_cls), 
                new_changes=new_cls_for_level
            ))
...
if __name__ == "__main__":
    ...
    test_defs = [
        TestDefinition(
            id=i,
            p_affected=0.001,
            # 0.0 broken, 0.995 flaky, 1.0 pass
    ...
    print(f"{'Max Batch':<10} | {'Throughput (CLs/tick)':<22} | {'Final Queue Size'}")
    print("-" * 55)

    for max_mb in [1, 2, 4, 8, 16, 32, 64]:
        sq = PipelinedSubmitQueue(test_defs, pipeline_depth=FIXED_DEPTH, max_minibatch_size=max_mb)
        
        # Target enough pending work to saturate the pipeline at this max_mb size
        # If max_mb is huge, we need many pending CLs to actually see it utilized.
        pending_target = FIXED_DEPTH * max_mb * 2 
        
        submitted_total = 0
        sq.add_changes(pending_target)

        for _ in range(N_ITER):
            # Keep queue topped up so we can actually test the batch sizes
            if len(sq.pending_changes) < pending_target:
                 sq.add_changes(FIXED_DEPTH * max_mb)

            submitted_total += sq.step()

        throughput = submitted_total / N_ITER
        print(f"{max_mb:<10} | {throughput:<22.2f} | {len(sq.pending_changes)}")